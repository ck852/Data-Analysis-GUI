...
def compare_csv_files(
    generated_path: Path, golden_path: Path, tolerance: float = 1e-5
) -> Tuple[bool, str]:
    """
            ...
            for col_idx, (gen_val, gold_val) in enumerate(zip(gen_row, gold_row)):
                if isinstance(gen_val, float) and isinstance(gold_val, float):
                    ...
                    if np.abs(gen_val - gold_val) >= tolerance:
                        return (
                            False,
                            f"Value mismatch at row {row_idx}, col {col_idx}: {gen_val} vs {gold_val} (diff: {np.abs(gen_val - gold_val)})",
                        )
                else:
                    if gen_val != gold_val:
                        return (
...
class TestSwappedPeaksBase(ABC):
    ...
    def _test_single_peak_mode(self, batch_processor, temp_output_dir, peak_mode: str):
        ...
        assert (
            len(batch_result.successful_results) > 0
        ), f"No successful batch results for {peak_mode} mode"
        assert (
        ...
        export_result = batch_processor.export_results(
            batch_result=batch_result, output_dir=str(peak_output_dir)
        )

        assert (
            export_result.success_count > 0
        ), f"No files exported successfully for {peak_mode} mode"
        ...
        if not golden_dir.exists():
            pytest.skip(
                f"Golden data directory not found for {peak_mode}: {golden_dir}"
            )
        ...
        if comparison_failures:
            failure_report = (
                f"CSV comparison failed for {peak_mode} mode:\n"
    ...
    def test_controller_workflow_with_peak_mode(self, temp_output_dir, peak_mode):
        ...
        # Create output subdirectory for this peak mode
        peak_output_dir = (
            temp_output_dir / f"controller_{get_peak_mode_folder_name(peak_mode)}"
        )
        peak_output_dir.mkdir(parents=True, exist_ok=True)

        # Export results
        export_result = controller.export_batch_results(
            batch_result=batch_result, output_directory=str(peak_output_dir)
        )
    ...
    def test_peak_mode_normalization(self):
        ...
        for input_mode, expected_normalized in test_cases:
            params = AnalysisParameters(
                ...
                stimulus_period=1000.0,
                x_axis=AxisConfig(
                    measure="Peak", channel="Voltage", peak_type=input_mode
                ),
                y_axis=AxisConfig(
                    measure="Peak", channel="Current", peak_type=input_mode
                ),
                channel_config={},
            )
