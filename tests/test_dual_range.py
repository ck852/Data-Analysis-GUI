...
class TestDualRangeAnalysis:
    ...
    def analyze_file(self, controller: ApplicationController, 
                     file_path: Path, 
        ...
        assert load_result.success, f"Failed to load {file_path}: {load_result.error_message}"
        ...
        assert sweep_count == 234, f"Expected 234 sweeps, got {sweep_count} for {file_path}"
        ...
        assert analysis_result.success, f"Analysis failed for {file_path}: {analysis_result.error_message}"
        assert analysis_result.data is not None, "Analysis data is None"
        
        # Verify dual range structure
        assert analysis_result.data.use_dual_range == True, "Dual range should be enabled"
        assert len(analysis_result.data.x_data) == 234, f"Expected 234 x-values, got {len(analysis_result.data.x_data)}"
        ...
        assert export_result.success, f"Export failed for {file_path}: {export_result.error_message}"
        assert os.path.exists(output_path), f"Output file not created: {output_path}"
        assert export_result.records_exported > 0, "No records were exported"
        
        return True, output_path
    
    def test_dual_range_abf_file(self, controller, test_data_path, golden_data_path, dual_range_parameters):
        """
        ...
        assert abf_reference.exists(), f"ABF golden reference not found: {abf_reference}"
    ...
    def test_dual_range_validation(self, controller, test_data_path, dual_range_parameters):
        ...
        # Create parameters
        params = self.create_parameters_from_gui_state(controller, dual_range_parameters)
        
        # Verify parameter validation
        assert params.range1_start < params.range1_end, "Range 1 start should be before end"
        assert params.range2_start < params.range2_end, "Range 2 start should be before end"
        assert params.range1_end < params.range2_start, "Range 1 should end before Range 2 starts"
        ...
        assert abs(range1_mean - range2_mean) > 1e-3, \
